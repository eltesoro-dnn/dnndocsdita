...
:. This changes the URL to lowercase for SEO purposes. (Code from Nathan Rover. Make it the last rule.)
echo                [[rule name="LowerCaseRule1" stopProcessing="true"]] >> %_log%
echo                    [[match url="[A-Z]" ignoreCase="false" /]] >> %_log%
echo                    [[action type="Redirect" url="{ToLower:{URL}}" /]] >> %_log%
echo                [[/rule]] >> %_log%
...
echo ----------------------------------------------------------------------------------------------
echo IMPORTANT: Edit %_log% to replace the double square brackets [[ ]] and make it a valid XML file.
echo            Then copy the web.config file to the _content folder.
echo ----------------------------------------------------------------------------------------------
