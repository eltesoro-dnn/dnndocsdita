...
:. Valid options: common-html, docbook, eclipsecontent, eclipsehelp, html5, htmlhelp, javahelp, odt, pdf, pdf2, tocjs, troff, wordrtf, xhtml.
set _transtype=html5
for %%v in ( common-html docbook html5 htmlhelp pdf pdf2 xhtml ) do  if _%1_ EQU _%%v_  set _transtype=%1

if _%_gitdir%_      EQU __    set _gitdir=w:\.
if _%_blddir%_      EQU __    set _blddir=v:\bld
if _%_tgtdir%_      EQU __    set _tgtdir=v:\output\%_transtype%
...
echo Deleting files in build directory %_blddir% ....
rd /s/q %_blddir%  >nul
md %_blddir%

echo Copying files required by the build ....
xcopy %_gitdir%\_content\*.dita*         %_blddir%\.                /s/v/y
xcopy %_gitdir%\_content\*.png           %_blddir%\.                /s/v/y
...
rd /s/q %DITA_HOME%\plugins\org.dnn.dc
...
xcopy %_gitdir%\_content\common\samples   %_tgtdir%\common\samples   /i/s/v/y
xcopy %_gitdir%\_content\common\img\*.png %_tgtdir%\common\img       /i/s/v/y
xcopy %_gitdir%\_themes\dnn\26D3F6_6_0.*  %_tgtdir%\_theme           /i/s/v/y
xcopy %_gitdir%\_themes\dnn\*.jpg         %_tgtdir%\_theme           /i/s/v/y
xcopy %_gitdir%\_themes\dnn\*.js          %_tgtdir%\_theme           /i/s/v/y


:. runas /user:administrator /profile "%~f0\v2iis.bat %_tgtdir% c:\inetpub\wwwroot"
start %_tgtdir%
