...
function MkBlurb( [string] $prod, [string] $persona, [string] $menu1, [string] $menu2, [string] $tab1, [string] $tab2, [string] $imgprod )  {
    $menu1squa = $menu1.Replace( " ", "" )
    $menu2squa = $menu2.Replace( " ", "" )
    $tab1squa = $tab1.Replace( " ", "" )
    $tab2squa = ""
    $bpid = "pbtabs-$persona-$menu1squa-$menu2squa-$tab1squa-$prod"
    $imgfn = "scr-pbtabs-$persona-$menu1squa-$menu2squa-$tab1squa-$imgprod.png"
    $cascadealt = "$tab1"
    if ( IsValid $tab2 )  {
        $tab2squa = $tab2.Replace( " ", "" ).Replace( ".NET", "NET" )
        $bpid  = "pbtabs-$persona-$menu1squa-$menu2squa-$tab1squa-$tab2squa-$prod"
        $imgfn = "scr-pbtabs-$persona-$menu1squa-$menu2squa-$tab1squa-$tab2squa-$imgprod.png"
    ...
    Write-Output( "           <!-- <li outputclass=""step stepexpand""><ph outputclass=""cmd"" conkeyref=""k-bppbtabs/ph-$bpid""></ph><image conkeyref=""k-bppbtabs/image-$bpid""></image></li> -->" )
        ...
        foreach ( $ln in $csv )  {
            if ( IsValid $ln.tab1 )  {
                if (( $ln.$_ -eq $_ ) -or ( $ln.host -eq $_ ))  {
                    MkBlurb $prod $_ $ln.menu1 $ln.menu2 $ln.tab1 $ln.tab2 $ln.imgprod | Out-File -Append $outfile
                }
            }
        }
    }
