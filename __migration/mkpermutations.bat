...
for /f "usebackq" %%v in ( `dir out\jobs-images-pbar*.out /b` ) do type out\%%v >> out\jobs-images-pbar.out
...
for /f "usebackq" %%v in ( `dir out\jobs-steps-pbar*.out /b` ) do type out\%%v >> out\jobs-steps-pbar.out
...
for /f "usebackq" %%v in ( `dir out\jobs-*-pbar.out   /b` ) do start windiff json\%%~nv.json out\%%~nv.out
for /f "usebackq" %%v in ( `dir out\jobs-*-pbtabs.out /b` ) do start windiff json\%%~nv.json out\%%~nv.out
